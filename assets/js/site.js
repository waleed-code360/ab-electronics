...
function waLink(product){
  const text = product
    ? `Hi AB Electronics, please share the current price and availability of ${product.model} (${product.name}).`
...
function productCard(p){
  return `<article class="product-card">
    <a class="product-img" href="product.html?id=${encodeURIComponent(p.id)}">
      <img src="${p.image}" referrerpolicy="no-referrer" alt="${p.model} ${p.name}" onerror="this.style.display='none';this.parentElement.innerHTML='<div style=&quot;text-align:center;color:#8995a3;font-size:13px&quot;>Product image unavailable</div>'">
    </a>
    <div class="product-body">
      <span class="product-cat">${p.category}</span>
      <h3>${p.name}</h3>
      <div class="model">${p.model}</div>
      <div class="tags">${p.highlights.slice(0,3).map(x=>`<span>${x}</span>`).join("")}</div>
...
function initProducts(){
  ...
  function render(){
    ...
    const items=PRODUCTS.filter(p=>(category==="All"||p.category===category)&&(!term||`${p.model} ${p.name} ${p.category} ${p.highlights.join(" ")}`.toLowerCase().includes(term)));
...
function initProduct(){
  ...
  document.title=`${p.model} | AB Electronics`;
  root.innerHTML=`<div class="product-detail">
    <div class="detail-image"><img src="${p.image}" referrerpolicy="no-referrer" alt="${p.model} ${p.name}"></div>
    <div class="detail-copy">
      <div class="category">${p.category}</div>
      <h1>${p.name}</h1>
      <div class="detail-model">${p.model}</div>
      <div class="detail-tags">${p.highlights.map(x=>`<span>${x}</span>`).join("")}</div>
      ...
      <h2 style="font-size:22px;margin-top:28px">Specifications</h2>
      <table class="spec-table">${Object.entries(p.specs).map(([k,v])=>`<tr><td>${k}</td><td>${v}</td></tr>`).join("")}</table>
      <p class="notice">Product details are based on Haier Pakistan's current product pages. Final stock, current price, warranty card and availability should be confirmed with AB Electronics before purchase.</p>
    </div>
  </div>`;
