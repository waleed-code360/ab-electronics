...
for(const [category,url] of categories){
  ...
  for(const item of unique){
    // Always open the official product page so title and model are kept separate.
    const p=await context.newPage();
    let productName='', model='', imageUrl=item.image, highlights=[];
    try{
      await p.goto(item.href,{waitUntil:'domcontentloaded',timeout:45000});
      await p.waitForTimeout(450);
      productName=clean(await p.locator('h1').first().textContent().catch(()=>''));
      const pageTitle=clean(await p.title());
      if(productName && pageTitle){
        const prefix=pageTitle.replace(/^Haier\s+/i,'');
        const suffix=`-${productName}-Haier Pakistan`;
        const idx=prefix.lastIndexOf(suffix);
        if(idx>0) model=clean(prefix.slice(0,idx));
      }
      if(!model){
        const slugModel=decodeURIComponent(new URL(item.href).pathname.split('/').filter(Boolean).pop().replace(/\.shtml$/,''));
        model=slugModel.toUpperCase();
      }
      if(!productName){
        productName=clean(item.text.replace(/^Hot\s+New\s+/i,'').replace(/Add\s+to\s+Compare/ig,''));
      }
      // Prefer category-card image (exact card/product mapping). Only fall back to a large official page image.
      if(!imageUrl){
        imageUrl=await p.locator('img').evaluateAll(imgs=>{
          const c=imgs.map((img,index)=>({src:img.currentSrc||img.src||'',area:(img.naturalWidth||0)*(img.naturalHeight||0),alt:img.alt||'',index}))
            .filter(x=>x.src && /image\.haier\.com/i.test(x.src) && !/logo|icon/i.test(x.src));
          return c[0]?.src||'';
        }).catch(()=> '');
      }
      highlights=await p.locator('h1').first().evaluate(h1=>{
        const out=[];let node=h1.parentElement;
        if(!node) return out;
        const texts=[...node.querySelectorAll('img[alt]')].map(i=>(i.getAttribute('alt')||'').trim()).filter(Boolean);
        for(const x of texts){if(!/^(image|icon|logo)$/i.test(x) && !out.includes(x)) out.push(x);if(out.length===3)break}
        return out;
      }).catch(()=>[]);
    }finally{await p.close()}

    if(!productName || !model) throw new Error(`Could not separate official product name/model for ${item.href}`);
    if(!imageUrl) throw new Error(`No official product image found for ${category}: ${model} (${item.href})`);
    ...
    collected.push({id,category,officialTitle:`${productName} — ${model}`,name:productName,model,image:`assets/products/${fileName}`,highlights,specs:{},source:item.href});
    console.log(`  ✓ ${productName} | ${model}`);
  }
}
